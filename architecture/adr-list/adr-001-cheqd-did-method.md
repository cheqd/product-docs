...
Schemas and Credential Definitions are also not covered under the scope of this ADR. Their implementation is covered in [ADR 002: *DID-Linked Resources*](adr-002-did-linked-resources.md).
...
A `did:cheqd` DID **must** be unique. The syntax of the `unique-id` component **may** be defined as a [Universally Unique Identifier (UUID)](https://en.wikipedia.org/wiki/Universally_unique_identifier) generated by the creator(s) of the DID. UUIDs are the preferred unique identifier format for cheqd network.
...
1. **`@context`** (optional): A list of strings with links or JSONs for
